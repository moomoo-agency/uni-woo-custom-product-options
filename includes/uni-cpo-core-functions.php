...
function uni_cpo_process_formula_scheme( $variables, $data, $purpose = 'price' )
{
    
    if ( 'price' === $purpose ) {
        $scheme_data = $data['formula_data']['formula_scheme'];
    } elseif ( 'weight' === $purpose ) {
        $scheme_data = $data['weight_data']['weight_scheme'];
...
function uni_cpo_add_to_cart_button( $link, $product )
{
    ...
    if ( isset( $product_custom['_uni_cpo_display_options_enable'][0] ) && $product_custom['_uni_cpo_display_options_enable'][0] == true ) {
        $link = sprintf(
...
function uni_cpo_calculate_price_in_cart( $cart_item_data, $product_id )
{
    try {
        ...
        $is_calc_weight = ( 'on' === $product_data['weight_data']['weight_enable'] ? true : false );
        $filtered_form_data = array_filter( $form_data, function ( $k ) use( $form_data ) {
            return false !== strpos( $k, UniCpo()->get_var_slug() ) && !empty($form_data[$k]);
        }, ARRAY_FILTER_USE_KEY );
...
function uni_cpo_re_add_cpo_item_data( $item_data, $raw_data )
{
    ...
    if ( is_object( $raw_data ) ) {
        foreach ( $raw_data as $meta ) {
            
            if ( false !== strpos( $meta->key, '_cpo' ) ) {
                $meta_key_new = ltrim( $meta->key, '_' );
                $item_data[$meta_key_new] = $meta->value;
            }
